...
void SEVENSEGMENTComponent::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "  Number of Digits: %u", this->num_digits);
...
void SEVENSEGMENTComponent::send_byte_(uint8_t a_register, uint8_t data) {
  this->write_byte(a_register);
  this->write_byte(data);
}

void SEVENSEGMENTComponent::send_to_all_(uint8_t a_register, uint8_t data) {
  this->enable();
...
uint8_t SEVENSEGMENTComponent::print(uint8_t start_pos, const char *str) {
  ...
  for (; *str != '\0'; str++) {
    ...
    if (data == SEVENSEG_UNKNOWN_CHAR) {
      ESP_LOGW(TAG, "Encountered character '%c' with no 7 segment representation while translating string!", *str);
    }
    if (*str == '.') {
      if (pos != start_pos)
        pos--;
      ...
      if (pos >= this->num_chips_ * 8) {
        ESP_LOGE(TAG, "7 segment String is too long for the display!");
        break;
      }
